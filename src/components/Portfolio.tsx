...
export function Portfolio() {
  ...
  const availableCash = useTradingStore(state => state.availableFunds);
  const resetPortfolio = useTradingStore(state => state.resetPortfolio);
  const getStats = useTradingStore(state => state.getStats);
  const stats = getStats();

  const totalStockValue = portfolio.reduce((sum, p) => sum + p.currentPrice * p.quantity, 0);
  const totalProfit = portfolio.reduce((sum, p) => sum + (p.currentPrice - p.avgCost) * p.quantity, 0);
  ...
  return (
    <>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {portfolio.map((position) => {
                    // 从股票列表中获取最新日期的价格
                    const stockRecords = stocks.filter(s => s.code === position.stockCode);
                    // 按日期排序，取最新的
                    const latestStock = stockRecords.sort((a, b) => new Date(b.dateTag).getTime() - new Date(a.dateTag).getTime())[0];
                    const latestPrice = latestStock?.price || position.currentPrice;
                    
                    const profit = (latestPrice - position.avgCost) * position.quantity;
                    const profitPercent = position.avgCost > 0 ? ((latestPrice - position.avgCost) / position.avgCost) * 100 : 0;
                    const marketValue = latestPrice * position.quantity;

                    return (
                      <tr key={position.id || position.stockCode} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="py-3 px-4 text-right">¥{latestPrice.toFixed(2)}</td>
      ...
      <ConfirmDialog
        ...
        title="确认重置"
        message="确定要重置投资组合吗？所有持仓和交易记录将被清除。"
      />
    </>
  );
}
